...
struct Upgrade: Identifiable, Hashable {
    ...
    init(
        ...
        // Upgrade templates live as `static let`s in UpgradeManager, so each upgrade's
        // ID is created once and stays stable for the lifetime of the app session. The
        // save system keys on upgrade name, not ID, so no cross-launch ID stability is
        // required here.
        self.id = id
